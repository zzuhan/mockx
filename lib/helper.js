...
function getRulesByProjectId(projectIds){
  var rules = [];

  if(!projectIds.length) return rules;

  try {
    var res = request('GET', 'http://10.218.131.225/api/client/rules?project_ids=' + projectIds.join(','), {
      timeout: 3000
    });

  } catch(e) {
    console.log('[Mockx Warning] dip根据projectId获取, 确认mockx服务是否良好');
    return rules;
  }
