...
function MockBullet(req, config, param, cb, res) {
  ...
  if(config.remote == 'self') {
    ...
    if(!this.hosts[req.headers.host]) {
      throw new Error('请把此host配置' + req.headers.host + '写入flex-hosts配置中');
    }

    var options = {
      // 对应req.headers.host对应的线上IP
      // TODO 有可能其未填写，需要提示报错
      host: this.hosts[req.headers.host],
      // node http.IncomingMessage .url
      port: remoteUrlObj.port,
      // 参见 https://nodejs.org/api/http.html#http_message_url
      path: req.url,
      method: req.method,
      headers: req.headers
    }
