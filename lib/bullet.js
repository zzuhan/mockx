...
function MockBullet(req, config, param, cb) {
  ...
  this.base       = pathLib.join(process.cwd(), this.param.mockDir);
  // 正则路径的计算
  this.regMatched = null;

  if (helper.isRegExp(config.route||config.pathname)) {
    this.preProcessReg();
  }
...
MockBullet.prototype.preProcessReg = function () {
  var urlObject = urlLib.parse(this.req.url);
  this.regMatched = urlObject.pathname.match(this.config.route);
}
...
MockBullet.prototype._callback = function (err, data/* 如果是对象就需要处理，非对象不独处 */, statusCode, headers, targetPath, charset) {
  ...
  if(config.compare) {
    ...
    new MockBullet(this.req, config, this.param, function (err, mockData, targetPath, statusCode, headers, charset) {
      ...
      // 再返回之前的结果
       setTimeout(function () {
        _this.cb(err, JSON.stringify({result: 'compare成功'}), targetPath, statusCode, headers, charset);
...
MockBullet.prototype.handleByRemote = function () {
  ...
  // 生成的options
  var finalPath/*包含query*/, finalHost, finalPort, finalProtocol, finalHeaderHost;

  // 这个应该保持的是target中那个真实的host
  // TODO 改名，意思是targetDomain
  var targetDomain = '';
  ...
  if (config.remote == 'self' || config.remote == 'origin' || config.remote == 'original') {
    ...
    var remoteUrlObj = urlLib.parse(remoteUrl, true);
    // 如果127.0.0.1 则hostname还是使用原来的host，比如daily.taobao.net -> 127.0.0.1 则hostname还是daily.taobao.net而不是127.0.0.1，因为登录的js会根据hostname来转
    finalHost = remoteUrlObj.hostname !== '127.0.0.1' ? remoteUrlObj.hostname : reqUrlObj.hostname;
    finalPort = remoteUrlObj.port;

    var reqUrlQueryObj = querystring.parse(reqUrlObj.query);
    var remoteUrlQueryObj = querystring.parse(remoteUrlObj.query);
    var mergedQueryObj = merge(reqUrlQueryObj, remoteUrlQueryObj);
    // 因为querystring.stringify时，如果value是空，会变成?a=&b=新的就变成了?a&b=
    function stringifyQuery(queryObj){
      ...
      for(var key in queryObj) {
        ret += '&' + key + (queryObj[key] ? eq + queryObj[key] : '');
      }
      return ret.replace(/^&/, '');
    }
    var search = stringifyQuery(mergedQueryObj);
    // var search =  querystring.stringify(mergedQueryObj, null, null, {encodeURIComponent: function(value) {
    //   return value;
    // }})
  ...
  var options = {
    ...
    method: req.method,
    headers: req.headers,
    // 关闭ssl的强校验，因为在本地如果有Hosts配置，ssl校验有问题
    rejectUnauthorized: false
  }

  // headers的host应该替换为新的url的host
  options.headers.host = targetDomain;

  if(config.requestHeaders) {
    options.headers = merge(options.headers, config.requestHeaders);
  }
