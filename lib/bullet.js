...
function MockBullet(req, config, param, cb) {
  ...
  async.parallel(queue, function (err, results) {
    ...
    if (!jsonRet && !jsDataRet && !remoteRet) {
      cb(err, "[Fake Path]");
    }
    else {
      if (config.jsonp) {
...
MockBullet.prototype.handlers = {
  ...
  // 先只支持带上get的参数，
  // 
  // TODO 服务端返回jsonp的处理，通过将其eval，得出其中的json串，这个是比较靠谱的，正则从其中提取不太靠谱
  // 
    ...
    if (target !== 'self') {
      ...
      fetch.request(finalUrl, hosts, function (err, buff) {
        if (!err) {
          ...
          var charset = isUtf8(buff) ? 'utf-8' : 'gbk',
            decodedStr = iconv.decode(buff, charset);
            jsonData = {};
          ...
          // 假设是jsonp，进行解析
            ...
            var f = new Function(callbackName, decodedStr);
            f(respondRet);
          }

          function respondRet(jsonData) {
              callback(null, {
                data: jsonData,
                path: finalUrl,
                charset: charset
...
function jsonp2json(str){
  ...
  return str.substring(firstBracketIndex+1, str.length -2);
}
