...
MockBullet.prototype.handleByFile = function () {
  ...
  if (stats.isFile()) {
    res.writeHead(200, {'Content-Type': config.fileType});

    var fileStream = fs.createReadStream(file);
    fileStream.pipe(res);
    this.callback(null, file);
  } else {
    this.callback(file + ' Not exists, or Not a valid file');
  }
}
...
MockBullet.prototype.callback = function (err, targetPath, charset, data/* 如果是对象就需要处理，非对象不独处 */, statusCode, headers) {
  ...
  if (this.config.postProcess) {
    var postProcessFile = pathLib.join(this.base, this.config.postProcess);
    delete require.cache[postProcessFile];
    // console.log('postFile', postProcessFile);
    require(postProcessFile)(headers, data, function (newHeaders, newData) {
      // setTimeout(function () {
      _this.cb(err, targetPath, charset, newData, statusCode, newHeaders);
...
MockBullet.prototype.handleByRemote = function () {
  ...
  // 如果remote字段写成 www.baidu.com/xxx 则protocol我们补全为http:
  ...
  // console.log('protocol is ' + finalProtocol);
  // console.log(typeof reqAgents[finalProtocol]);
  
  // 发送请求
  var proxyReq = reqAgents[finalProtocol]['request'](options, function (proxyRes) {
    // console.log('invoke there');
    // var buf = [];
    ...
    //   var charset = isUtf8(body) ? 'utf8' : 'gbk';

    //   body = iconv.decode(body, charset);

    //   _this.callback(null, 'xxx', charset, body, proxyRes.statusCode, proxyRes.headers);
    // })
    try {
      _this.res.writeHead(proxyRes.statusCode, proxyRes.headers);
      proxyRes.pipe(_this.res);
    } catch (e) {
      console.log(e);
      console.log(e.stack);
    }
