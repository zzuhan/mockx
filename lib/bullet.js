  ...
  var queue = [
    (function (route, file) {
      return function (callback) {
        _this.jsonHandler(req.url, route, file, callback);
      }
    })(config.route, config.json),

    (function (route, file) {
      return function (callback) {
        _this.jsDataHandler(req.url, route, file, callback);
      }
    })(config.route, config.jsData),

    (function (route, file) {
      return function (callback) {
        _this.remoteHandler(req.url, route, file, callback);
      }
    })(config.route, config.remote)
  ];
...
['json', 'jsData', 'remote'].forEach(function (type) {
  MockBullet.prototype[type + 'Handler'] = function (url, route, target, callback) {
    // 如果不存在，返回null就行了
    if (!target || target.indexOf('#') == 0) {
      callback(null, null);
    } else {
      if (typeof route != "string") {
        var matched = url.match(route);
        if (matched) {
...
MockBullet.prototype.handlers = {
  ...
  remote: function (target, callback, req, hosts) {
    ...
    if (target !== 'self') {
      ...
      fetch.request(finalUrl, hosts, function (err, buff, nsres) {
        if (!err) {
