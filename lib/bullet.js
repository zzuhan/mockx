...
function MockBullet(req, res, config, param, cb) {
  this.cb         = cb;
  this.config     = config;
  this.req        = req;
  this.res        = res;
...
MockBullet.prototype._callback = function (err, data/* 如果是对象就需要处理，非对象不独处 */, statusCode, headers, targetPath, charset) {
  ...
  // 还是在这个阶段做下比对把
  // 通过将请求发送为self的形式，来拿到结果，然后比对
  // req, res, config, param, callback

  if(config.compare) {
    config.remote = 'self';
    config.compare = false;
    new MockBullet(this.req, this.res, config, this.param, function (err, onlineData, targetPath, statusCode, headers, charset) {
      // 可以对两者的结果进行compare了哈哈。
      compareWrap(data, onlineData);
    });
...
MockBullet.prototype.handleByJSON = function () {
  ...
  try {
    ...
    this._callback(null, data, null, null, fullpath, charset);
...
MockBullet.prototype.handleByJSONText = function () {
  ...
  try {
    ...
    this._callback(null, jsontext, 200, null, 'jsontext', 'utf8');
