...
Mock.prototype.handle = function (req, res, next) {
  ...
  if (!config) {
    ...
    new MockBullet(req, config, this.param, function (err, targetPath, charset, dataStr, statusCode, headers) {
      // log
      ...
      // console.log('bullet return dataStr is' + dataStr);
      // console.log('bullet return dataStr with encoding' + charset);

      if (!err) {
        if(dataStr){
          // 因为内容被我更改了，content-length跟新的内容长度不符合
          ...
          // console.log('data is ', res.buffer);
          res.end(res.buffer);
        }
      }
      else {
