...
Mock.prototype.handle = function (req, res, next) {
  ...
  if (!config) {
    ...
    new MockBullet(req, config, this.param, function (err, targetPath, charset, data) {
      ...
      if (!err) {
        res.mockHeaders['content-type'] = "application/json;charset=" + charset;
        res.writeHead(200, {
          'Content-Type': res.mockHeaders['content-type']
        });
...
Mock.prototype._findMatchConfig = function (req) {
  ...
  this.configs.forEach(function (config) {
    ...
    if (checkMatch(removeLastBackslash(config.route), removeLastBackslash(path) ) && ifQueryMatch(config.data, reqData) && ifHostMatch(config.host, host)) {
      // 是用第一个匹配的，还是后面的强于前面的?
      // matchConfig = config;
