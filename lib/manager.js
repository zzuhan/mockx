...
Mock.prototype._loadConfig = function () {
  ...
  this.config = require(this.confFile);
  this.rules = this.config.rules;
  this.param.mockDir = this.config.mockDir;
  // 仅执行一次fetch dip，为了保证速度
  this.rules = this._compileDipConfig(this.rules);

  this._genRuntimeFile();
};

Mock.prototype._genRuntimeFile = function () {
  fs.writeFileSync(this.runtimeFile, "module.exports = " + JSON.stringify(this.config, null, 2) + ";");
...
Mock.prototype.handle = function (req, res, next) {
  ...
  if (!rule) {
    ...
    new MockBullet(req, res, rule, this.param, handleResponse);
