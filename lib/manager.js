...
function MockWrap(param, confDir, confFile, hosts){
  param = param || {};
...
Mock.prototype.lookUpDns = function() {
...
Mock.prototype.refreshCache = function () {
  if(this.config.enableDip){
    this.refreshDipCache();
  }
  if(this.config.enableMockx){
...
Mock.prototype.handle = function (req, res, next) {
  ...
  // combo的情况呢？
  // http://c-assets.tmall.net/ju/seal/0.1.1/??vendor.js,pages/sellerIndex/index.js
  // http://c-assets.tmall.net/ju/seal/0.1.1/vendor.js
  // http://c-assets.tmall.net/ju/seal/0.1.1/pages/sellerIndex/index.js
  } else if(req.url.indexOf('??') !== -1) {
    var urlObj = urlLib.parse(req.url);

    this.comboRequest(req, res, next);
  ...
  if (!rule) {
    ...
    function handleResponse(err, dataStr, targetPath, statusCode, headers, charset) {
      ...
      if(rule.headers || rule.responseHeaders) {
        headers = merge(headers, rule.headers || rule.responseHeaders);
      }
...
Mock.prototype.comboRequest = function (req, res, next) {
  console.log('hint combo');
  // 还必须保持好顺序 1 2 3 
  console.log(req.url);
  ...
  var urlObj = urlLib.parse(fullUrl);
  ...
  files.forEach(function(file) {
    ...
    filePath = urljoin(filePath, file)
    filesPath.push(filePath);
  })

  var stack = new Stack(pkgName);
  stack.request((req.connection.encrypted ? "https" : "http") + "://" + req.headers.host, req.url);
  stack.info(filesPath, "combo files");
...
Mock.prototype._findMatchRule = function (req) {
  ...
  this.config.rules.forEach(function (rule) {
    ...
    // 过去的route模式
    }  if (_h.checkMatch(_h.removeLastBackslash(rule.route||rule.pathname), _h.removeLastBackslash(path) ) && _h.ifQueryMatch(rule.data||rule.query, reqData) && _h.ifHostMatch(rule.host, host)) {
      // 是用第一个匹配的，还是后面的强于前面的?
      // matchConfig = rule;
      matchRules.push(rule);
    }
  ...
  matchRules.forEach(function (rule) {
    if(_h.isRegExp(rule.route||rule.pathname)) {
  ...
  if(matchRule) {
    matchRule.route = matchRule.route || matchRule.pathname;
    matchRule.data = matchRule.data || matchRule.query;
  }
