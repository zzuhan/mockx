...
function Mock(params, confDir) {
  ...
  this.debugFile = pathLib.join(confDir, 'mockx.debug.js');

  this.hosts = params.hosts;
  this._loadConfig();
  this._genDebugFile(this.configs);
}
...
Mock.prototype._genDebugFile = function(configs){
  // if (!fs.existsSync(this.debugFile)) {
  //   fs.writeFileSync(this.debugFile, "module.exports = [];");
  //   fs.chmod(this.debugFile, 0777);
  // }
  fs.writeFileSync(this.debugFile, "module.exports = " + JSON.stringify(configs) + ";")
}
...
Mock.prototype._preProcess = function (configs) {
  ...
  configs.forEach(function (config) {
    if(config.dipApp){
      configs = configs.concat(getSchemasByAppId(config.dipApp));
    }
  });
  ...
  configs.forEach(function (config) {
    if(config.dipSchema) {
      config = getConfigBySchemaId(config.dipSchema, config);
    }
  });
...
function getConfigBySchemaId(schemaId, thisConfig){
  var schema = {};
  ...
  if(schema.meta && schema.meta.uri) {
    ...
    thisConfig.remote = thisConfig.remote || 'http://dip.alibaba-inc.com/api/v2/services/schema/mock/' + schemaId;
  }

  return thisConfig;
}

function getSchemasByAppId(appId) {
  var addConfigs = [];
  var schemas;
