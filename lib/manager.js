...
Mock.prototype._loadConfig = function (forceUpdate) {
  ...
  if(this.config.enableDip) {
    var rules = this._compileDipConfig(this.rules, forceUpdate);
    this.rules = this.rules.concat(rules);
  }
  if(this.config.enableMockx) {
    var rules = this._compileMockxConfig(this.config.projectId, forceUpdate);
    this.rules = this.rules.concat(rules);
  }
...
Mock.prototype._compileMockxConfig = function (projectId, forceUpdate) {
  // 返回cache
  if(this.mockxCompiled && !forceUpdate) {
    return this.mockxCache;
  } 

  var _this = this;

  var rules = _h.getRulesByProjectId(projectId);

  this.mockxCache = rules;
  this.mockxCompiled = true;
...
Mock.prototype._findMatchRule = function (req) {
  ...
  this.rules.forEach(function (rule) {
    ...
    // console.log(_h.ifHostMatch(rule.host, host));
    // console.log(rule.host, host);
