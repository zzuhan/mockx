...
Mock.prototype.loadConfig = function () {
	if(!fs.existsSync(this.confFile)) {
	    fs.writeFileSync(this.confFile, "[{}]");
...
Mock.prototype.findMatchConfig = function (req) {
	...
	this.configs.forEach(function (config) {
		...
		if( checkMatch(config.route, urlMeta.pathname) && ifQueryMatch(config.query || {}, urlMeta.query || {}) ) {
	...
	function ifQueryMatch(queryConfig, reqQuery){
		for(var key in queryConfig){
			if(!checkMatch(queryConfig[key], reqQuery[key])) {
				return false;
			}
		}
		return true;
