...
Mock.prototype.handle = function (req, res, next) {
  ...
  new MockBullet(req, config, this.param, function (err, data, targetPath) {
    if (!err) {
      // 根据结果，判断调用res还是next
      ...
      this.trace.response(targetPath);
...
Mock.prototype._findMatchConfig = function (req) {
  ...
  if (matchConfig && this.trace) {
    this.trace.request((req.connection.encrypted ? "https" : "http") + "://" + host, req.url);
    this.trace.info(matchConfig, "Target Config");
  }
